...
import { fmtUsdc, roundHalfUp, dayInTz, selectRows, periodLabel, banner, buildFooter, toolVersion } from './util.js';

export function valuedRows(db, cfg, period, to = null) {
  const fiatPair = `USD/${cfg.baseCurrency}`;
  return selectRows(db, period, to, cfg.timezone).map((r) => {
...
export function valuationReport(db, cfg, period, { incomplete = [], to = null } = {}) {
